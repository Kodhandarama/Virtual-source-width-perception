% Objective: Performance comparison of DESA-1 and ZC-AM/FM techniques for sinusoidal AM and sinusoidal FM
% estimation
...
N=512;
n=0:1:N-1;
am=1+0.75*sin(0.06125*n);
beta=4;
pm=pi*n/2+beta*sin(pi*n/100+pi/4);
fm=pi/2+beta*cos(pi*n/100+pi/4)*(pi/100);
fm=fm/(2*pi);
win=hamming(48)';
win=win/max(win);
...
L=256;			% bandpass filter impulse response length
b=fir1(L,[0.15 0.35]*2,'DC-0');
Q=24;t=Q+1:N-Q;

% Arrays to sore the CMSE results
% 1 for AM, 2 for FM
Dcmse1=[];Dcmse2=[];
Zcmse1=[];Zcmse2=[];

% SNRs (fine spacing at low SNR, large spacing at high SNR)
% non-uniform resolution was used to identify the low SNR threshold accurately
...
% bandpass random noise generation
r=randn(1,N);
...
save twotechs1		% save for future use
