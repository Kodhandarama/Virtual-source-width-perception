...
% Relevant reading: Sec.6.2.1, Page 113
...
% AM-FM signal generation (sinusoidal AM and sinusoidal FM)
...
% In the following: 1 for AM, 2 for FM
% Arrays to store the cumulative mean square error for the various techniques 
Dcmse1=[];Dcmse2=[];
Zcmse1=[];Zcmse2=[];
Hcmse1=[];Hcmse2=[];
Acmse1=[];Acmse2=[];
...
for trials=1:100	% No. of trials

SNR
trials

% Bandpass noisy signal generation
r=randn(1,N);
fr=conv(b,r);fr=fr(0.5*L+1:end-0.5*L);
fr=fr/std(fr);
noisy=amfm+sigstd*10^(-SNR/20)*fr;

% Estimation from various techniques
[d1,d2]=DESA1(noisy);
...
save perfcompcmse	% if you wish to use later
