function [a,x,f1_freq,sigma_freq] = gen_formant_noise(fs,f_from,sigma_from,e,formant_num)
    ...
    if(formant_num ==1)
        pk=pk1;
    elseif (formant_num ==2)
        pk=pk2;
     
    elseif (formant_num ==3)
        pk=pk3;
    
    elseif (formant_num ==4)
        pk=pk4;
    
    elseif (formant_num ==5)
        pk=pk5;
    
    elseif (formant_num ==6)
        pk=pk6;    
     
    elseif (formant_num ==7)
        pk=pk7;
    
    elseif (formant_num ==8)
        pk=pk8;
    
    elseif (formant_num ==9)
        pk=pk9;
    else
         pk =pk10;
    end

%     pk = [pk1 pk2 pk3 pk4 pk5 pk6 pk7 pk8 pk9 pk10];% pk6 pk7 pk8 pk9 pk10];

    a = poly(pk) ;

    homega = freqz(1,a,fs,'whole');
    f=1:fs;
%     plot(f,10*log10(abs(homega).^2),'r');
%     xlabel('Frequency(hz)')
%       ylabel('H(w)^2 (dB)')
%      hold on;
    x= filter(1,a,e);
