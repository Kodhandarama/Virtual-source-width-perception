...
function SNR_level = synthesis_vowel_with_noise_formants(filename,required_snr,vowel,modsigma,formant_num)
% load('A_filters.mat')
% load('U_filters.mat')
% load('I_filters.mat')
...
[~,mod_noise,~,~] = gen_formant_noise(fs,formantss,bandw.*modsigma,noise1,formant_num) ;
snrr = snr(e,mod_noise);
if(required_snr == 0)
   required_snr=required_snr+0.001; 
end
gamma = (snrr/required_snr);
alpha = ((rms(e)^2)/(rms(mod_noise)^2))^((gamma -1)/gamma);
scaling_factor = sqrt(alpha);
% pwelch(BPnoise * scaling_factor)
excitation = e + (mod_noise * scaling_factor);
check_snr = snr(e,mod_noise .* scaling_factor);
freq_lower = formantss(formant_num) - bandw(formant_num)/2;
freq_higher = formantss(formant_num) + bandw(formant_num)/2;
% 
% snrr = snr(e,noise1);
% exc = e + noise1;
% 
